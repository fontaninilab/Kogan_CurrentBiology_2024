function [responsive_neurons, responsive_neurons_indv_tastes, pvals_all] = ...
    find_responsive_neurons_v2(neural_data, time_stamps, baseline_window, resp_window)
    ...
    for z=1:size(neural_data.(groups(h)),2)
        for j=1:size(neural_data.(groups(h)),1)
            ...
            for i=1:size(neural_data.(groups(h)){j,z},1)
                ...
                avg_activity(i,1) = mean(neural_data.(groups(h)){j,z}(i,baseline_ts));
                avg_activity(i,2) = mean(neural_data.(groups(h)){j,z}(i,response_ts));
